function run(msg)
	...
	.."   *All users list:*\n"
	.."     /users"
	about_txt = "*TeleFir Robot* v"..bot_version.."\n\n   _- Send Message by SMS_\n   _- Add, Edit & Remove Caption_\n   _- Send Inline Keyboard_\n   _- Remove Forward Info_\n   _- Send Bold,Italic and Cod Text_\n   _- Send Hyper Link_\n   _- Timer for Post to Next Time_\n   _- & More..._\n\n`Made in TeleFir Team and Powered by csgroup Co. Group`"
	about_key = {{{text = "کانال رسمی" , url = "https://telegram.me/TeleFir"}},{{text = "ربات تیم" , url = "https://telegram.me/TeleFirBot"}},{{text = "پیام رسان" , url = "https://telegram.me/Troy72bot"}}}
...
`با استفاده از این کلید میتوانید نام کانال، یوزرنیم کانال، آی دی کانال، شماره ارتباطی با سامانه پیامکی، انقضای اکانت، آمار کانال، سازنده ی کانال، تعداد ادمینهای کانال، لیست ادمینهای کانال را در یک لیست ببینید.`]]
	...
	elseif msg.text == "مثال کیبرد اینلاین و مارک داون" then
		inlinekey = {{{text = "کانال تیم" , url = "https://telegram.me/TeleFir"}},{{text = "ربات تیم" , url = "https://telegram.me/TeleFirBot"}}}
		text = "مثال استایل نویس با فونتهای مختلف:\n*TeleFir Team*\nTeleFir Team\n_TeleFir Team_\n`TeleFir Team`\nتیم سی اس گروپ\n`تیم سی اس گروپ`\nمثال هایپر لینک:\n[تیم سی اس گروپ](telegram.me/TeleFir)\n[TeleFir Team](telegram.me/TeleFir)\nمثال کیبرد اینلاین:"
	...
	if msg.text:find('/add') and msg.from.id == sudo_id then
		...
		if usertarget then
			...
			return send_msg(sudo_id, "*/add 12345678,@TeleFir,9121234567*\n`/add [telegram id],[@channel],[number]`", true)
		end
	elseif msg.text:find('/edit') and msg.from.id == sudo_id then
		...
		if usertarget then
			...
			return send_msg(sudo_id, "*/edit 12345678,@TeleFir,9121234567*\n`/edit [telegram id],[@channel],[number]`", true)
		end
	...
	elseif msg.text == "مثال کیبرد اینلاین" then
		return send_inline(msg.from.id, "`تیتر کیبرد اینلاین با قابلیت استفاده از قابلیت مارک داون`", {{{text = "کانال تیم" , url = "https://telegram.me/TeleFir"}}})
	end
